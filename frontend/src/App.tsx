...
function ThemeSwitcher({ mobile }: { mobile?: boolean }) {
  ...
  return (
    <div className="dropdown dropdown-end">
      <button
        tabIndex={0}
        className="btn btn-ghost btn-xs gap-1 text-[10px] font-bold uppercase tracking-widest opacity-60 hover:opacity-100"
      >
        🎨 Theme
      </button>
      <ul
        tabIndex={0}
        className="dropdown-content menu p-2 bg-base-200 border border-base-300 rounded-md shadow-2xl w-48 z-50"
      >
        <li className="mb-2 border-b border-base-300 pb-2">
          <button
            onClick={toggleDark}
            className="text-xs flex items-center justify-between font-bold"
          >
...
function CourseDetailOverlay({
    ...
    fetch(`${API}/api/courses/${courseId}/browse`)
      ...
      .then((d) => {
        ...
        if (restorePath && d?.items) {
          ...
          const initExp: Record<string, boolean> = {};
          d?.items?.forEach((i: FileItem) => {
            if (i.type === "folder") initExp[i.path] = true;
          });
          setExpanded(initExp);
        }
      })
        ...
        return (
          <div key={item.path} className="w-full relative">
            ...
            >
              ...
              <FolderOpen
                size={16}
                className="text-primary shrink-0 relative z-10"
              />
  ...
  const openFile = async (file: FileItem) => {
    ...
    history.replaceState(
      {},
      "",
      `/${courseId}?file=${encodeURIComponent(file.path)}`,
    );
    if (file.name.toLowerCase().endsWith(".xlsx")) {
      setActiveFile({ ...file, type: "other" });
      setFileContent(null);
      return;
    }
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        const el = document.getElementById(
          `curriculum-item-${file.path.replace(/[^a-zA-Z0-9]/g, "-")}`,
        );
    ...
    <div className="fixed inset-0 z-[90] bg-base-100 flex flex-col">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col md:items-center md:text-center w-full">
              ...
              {data.subtitle && (
                <p className="text-xs md:text-sm font-medium text-white/80 mb-2 md:mb-8 leading-relaxed hidden md:block">
                  {data.subtitle}
                </p>
              )}
              {/* Stats Row */}
          ...
          <div
            ref={sidebarRef}
            className="flex-1 flex flex-col overflow-y-auto bg-base-100"
          >
            ...
            <div className="sticky top-0 z-10 bg-base-200/70 backdrop-blur-md">
              <div className="p-3 border-b border-base-300 flex items-center justify-between shadow-sm gap-2">
                <div
                  className="text-[11px] font-black uppercase tracking-widest opacity-70 truncate flex-1"
            ...
            {loading ? (
              ...
              <div className="flex-1">{renderCurriculum(items)}</div>
            )}
          </div>
        </div>
      )}
      ...
      {activeFile && (
        <div className="flex-1 flex overflow-hidden relative">
          ...
          <div
            ref={sidebarRef}
            className={`hidden md:flex ${sidebarCollapsed ? "w-0 overflow-hidden border-r-0" : "w-1/3 lg:w-1/4"} border-r border-base-300 flex-col overflow-y-auto bg-base-100 relative transition-all duration-300`}
          >
            {/* Sticky Header with hide-on-scroll */}
            <div className="sticky top-0 z-10 bg-base-200/70 backdrop-blur-md">
              <div className="p-3 border-b border-base-300 flex items-center justify-start shadow-sm gap-2">
                <button
                  ...
                  className="btn btn-circle btn-sm btn-ghost text-primary shrink-0"
                >
                  {sidebarCollapsed ? (
                    <PanelLeft size={16} />
                  ) : (
                ...
                <button
                  onClick={onClose}
                  className="btn btn-circle btn-sm btn-ghost text-primary hover:bg-primary/10 shrink-0"
                >
                  <Home size={16} />
                </button>
                ...
                <div className="flex-1 flex flex-col bg-base-100 border border-base-300 rounded-xl overflow-hidden">
                      ...
                      <div className="flex-1 flex flex-col min-h-0 overflow-hidden bg-base-100">
                        {(activeFile.name.toLowerCase().endsWith(".html") ||
                          ...
                          <div className="flex border-b border-base-300 bg-base-200/50 p-2 gap-2 shrink-0">
                            <button
                              className={`btn btn-xs ${viewTab === "preview" ? "btn-primary" : "btn-ghost"}`}
                              onClick={() => setViewTab("preview")}
                            >
                              Preview
                            </button>
                            <button
                              className={`btn btn-xs ${viewTab === "code" ? "btn-primary" : "btn-ghost"}`}
                                  ...
                                  <>
                                    <thead>
                                      <tr className="bg-base-300 text-base-content">
                                        {header.map((cell, i) => (
                                          <th
                                            key={i}
                                            className="border-r border-base-200/50 last:border-r-0 px-4 py-3 sticky top-0 bg-base-300 z-10 whitespace-nowrap font-bold uppercase tracking-wider text-[10px] text-base-content/70"
                                          >
                                            {cell}
                                          </th>
                                        ))}
                                    ...
                                    <tbody>
                                      {body.map((row, i) => (
                                        <tr
                                          key={i}
                                          className="hover:bg-base-200/50 transition-colors border-b border-base-200/50 last:border-b-0"
                                        >
                                          {row.map((cell, j) => (
                                            <td
                                              key={j}
                                              className="border-r border-base-200/50 last:border-r-0 px-4 py-2.5 whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px] sm:max-w-xs"
                                              title={cell}
                                            >
                                              {cell}
                                            </td>
                  ...
                  {activeFile.type === "link" && (
                    <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center gap-6 p-12 text-center bg-base-200/30">
                      ...
                      {fileContent ? (
                        fileContent.url ? (
                          <>
                            <div className="max-w-md w-full bg-base-100 p-4 rounded-xl break-all text-sm border border-base-300 font-mono text-base-content/70">
                              {fileContent.url}
                            </div>
                            <a
                              ...
                              className="btn btn-primary px-8 py-3 h-auto text-xs font-bold uppercase tracking-widest gap-2"
                            >
                              Go to Webpage <ExternalLink size={16} />
                            </a>
                          </>
                        ) : (
                  ...
                  {activeFile.type === "other" && (
                    <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center gap-5 p-12 text-center bg-base-200/30">
                      ...
                      <a
                        href={`${API}/api/courses/${courseId}/file?path=${encodeURIComponent(activeFile.path)}`}
                        download={activeFile.name}
                        className="btn btn-primary text-xs font-bold uppercase tracking-widest gap-2"
                      >
  ...
  return (
    <div className="modal modal-open z-[100]">
      <div className="modal-box max-w-md bg-base-200 border border-base-300 p-0 shadow-2xl">
        <div className="p-4 border-b border-base-300 flex justify-between items-center">
          <h3 className="font-bold text-sm flex items-center gap-2">
            <GraduationCap size={16} className="text-primary" /> Add Course
          </h3>
          <button className="btn btn-ghost btn-xs btn-square" onClick={onClose}>
            <X size={14} />
          </button>
        ...
        <div className="p-5 flex flex-col gap-4">
          <div>
            <label className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1.5 block">
              Course Name
            </label>
            <input
              type="text"
              ...
              {COURSE_ICON_LIST.map((i) => (
                ...
                >
                  <CourseIcon
                    iconKey={i.key}
                    size={18}
                    className={icon === i.key ? "text-primary" : "opacity-50"}
                  />
                  <span className="text-[7px] font-bold uppercase tracking-tight opacity-60 leading-none">
                    {i.label}
                  </span>
                </button>
              ))}
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-base-100">
      ...
      <div className="p-4 max-w-4xl mx-auto">
        {courses.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <button
              onClick={() => setShowAdd(true)}
              className="btn btn-primary btn-sm gap-2 text-[10px] font-bold uppercase tracking-widest"
            >
      ...
      {/* Overlays */}
      {activeCourseId && (
        <CourseDetailOverlay courseId={activeCourseId} onClose={closeCourse} />
      )}
      {showAdd && (
        <AddCourseModal
